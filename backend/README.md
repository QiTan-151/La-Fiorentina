...
```json
{
  ...
  "price": 110000,
  "image_url": "https://api.lafiorentina.vn/uploads/abc123.jpg",
...
`image_url`, `description`, `sort_order`, `is_available` đều là tùy chọn.

### `POST /api/menu/upload` — admin, upload 1 file ảnh

Gửi dạng `multipart/form-data`, field tên `image`. Chấp nhận JPG/PNG/WEBP/GIF,
tối đa 5MB. Trả về `{ "url": "https://..." }` — dùng giá trị này làm
`image_url` khi tạo/sửa món. Ảnh cũ tự động bị xóa khỏi ổ đĩa khi món đổi
sang ảnh khác hoặc bị xóa hẳn.
...
> **Lưu ý về lưu trữ ảnh:** ảnh upload được lưu trực tiếp trên ổ đĩa server
> (thư mục `uploads/`), **cùng một loại rủi ro như database SQLite** — nếu
> deploy trên nền tảng có ổ đĩa tạm thời (vd Render không gắn Disk), ảnh có
> thể bị mất khi deploy lại. Xem `DEPLOY.md` để cấu hình lưu trữ bền vững.
