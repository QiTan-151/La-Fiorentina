...
cp .env.example .env
nano .env    # điền ADMIN_API_KEY, SMTP, Telegram, FRONTEND_ORIGIN thật
...
Database SQLite được lưu ở `./data/reservations.db` trên VPS (ngoài container)
nhờ volume trong `docker-compose.yml`, nên `docker compose down` rồi `up` lại
không mất dữ liệu.
...
- `RESERVATION_API_URL` trong `index.html`
...
- [ ] Đặt bàn thử từ form thật trên site — nhận được email/Telegram thông báo
- [ ] Đăng nhập thử `admin.html` bằng `ADMIN_API_KEY` thật
- [ ] Đổi `ADMIN_API_KEY` khác giá trị test đã dùng lúc phát triển
- [ ] Backup định kỳ file `reservations.db` (hoặc thư mục `./data`) — vd cron job copy sang nơi khác mỗi đêm
